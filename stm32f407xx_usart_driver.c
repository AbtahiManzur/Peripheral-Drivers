...
static void USART_SRDRRead(void);

static void USART_SRDRRead(USART_RegDef_t *pUSARTx)
{
	uint8_t dummyread;
...
void USART_ClearFlag(USART_RegDef_t *pUSARTx, uint16_t StatusFlagName)
{
	...
	if (StatusFlagName == USART_SR_PE)
	{
		//Wait for RXNE flag to be set
		while(!(pUSARTx->SR & (1 << USART_SR_RXNE)));
		USART_SRDRRead(pUSARTx);
	} else if (StatusFlagName == USART_SR_FE)
	{
		USART_SRDRRead(pUSARTx);
	} else if (StatusFlagName == USART_SR_NF)
	{
		USART_SRDRRead(pUSARTx);
	} else if (StatusFlagName == USART_SR_ORE)
	{
		USART_SRDRRead(pUSARTx);
	} else if (StatusFlagName == USART_SR_IDLE)
	{
		USART_SRDRRead(pUSARTx);
	...
	} else if (StatusFlagName == USART_SR_TC)
	{
		dummyread = pUSARTx->SR;
		pUSARTx->DR |= 0x0000;
	} else if (StatusFlagName == USART_SR_TXE)
	{
		pUSARTx->DR |= 0x0000;
	} else if (StatusFlagName == USART_SR_LBD)
	{
		pUSARTx->SR &= ~(1 << USART_SR_LBD);
	} else if (StatusFlagName == USART_SR_CTS)
	{
		if(pUSARTx->CR3 & (1 << USART_CR3_CTSE))
		{
